...
fn parse_cgcontrdp_section(at_part: &str) -> Result<Option<CgcontrdpOutputKind>, Error> {
    ...
    const {
        assert!(
            DNS_VEC_SIZE >= 2,
            "Vec holding the DNS should at least be able to contain 2 elements"
        )
    }
    ...
    // That's a lot of boilerplate to handle two different types...
...
impl<'a> Control<'a> {
    ...
    pub async fn status(&self) -> Result<Status, Error> {
        ...
        // According to Nordic's doc, the second IP should always be an IPv6.
        if let Some(ip) = ip2 {
            match IpAddr::from_str(ip).map_err(|_| Error::AddrParseError)? {
                IpAddr::V4(_) => {
        ...
        // When using an IPv6 stack, the modem returns 2 `+CGCONTRDP:` lines, one for IPv4 (even if we don't request it)
        // and one for IPv6. This is too long to be parsed by the at_commands crate so we split it.
        ...
        // First section can either be IPv4 or IPv6.
        if let Some(section) = sections.next() {
            ...
            // There should always be at least one +CGCONTRDP line.
            return Err(Error::AtParseError);
        }
